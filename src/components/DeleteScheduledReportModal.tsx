...
interface DeleteScheduledReportModalProps {
  isOpen: boolean
  onClose: () => void
...
export function DeleteScheduledReportModal({
  isOpen,
  onClose,
  onConfirmDelete,
  reportName
}: DeleteScheduledReportModalProps) {
  return (
    <AlertDialog open={isOpen} onOpenChange={onClose}>
      <AlertDialogContent className="sm:max-w-lg p-6 gap-0">
        ...
        <div className="flex items-start justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <AlertDialogTitle className="text-lg text-gray-900 leading-tight">
              Eliminar Programación - <span className="font-normal">{reportName}</span>
            </AlertDialogTitle>
          </div>
          <Button
            variant="ghost"
        ...
        <AlertDialogDescription className="text-gray-600 text-left mb-8 text-base">
          ¿Estás seguro que deseas eliminar la programación?
        </AlertDialogDescription>
