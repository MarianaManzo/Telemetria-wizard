...
import type { MainNavTopMenuProps, Notification, MenuItemKey } from './types';

const { Header } = Layout;

export default function MainNavTopMenu({
  selectedMenuItem = 'monitoreo',
  onMenuSelect,
  onNotificationSelect,
  onViewAllNotifications,
}: MainNavTopMenuProps) {
  const resolvedSelectedMenuItem = selectedMenuItem === 'home' ? 'monitoreo' : selectedMenuItem;
  ...
  const topMenuItems: { key: MenuItemKey; label: string }[] = [
    ...
    >
      ...
      >
        {topMenuItems.map((item) => {
          const isActive = resolvedSelectedMenuItem === item.key;
          return (
            <button
              key={item.key}
              type="button"
